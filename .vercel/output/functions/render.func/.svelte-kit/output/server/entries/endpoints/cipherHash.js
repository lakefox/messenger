...
function cipherHash1(msg, passPhrase) {
  msg = a2hex(msg);
...
function unCipherHash1(msg, passPhrase) {
  passPhrase = genPassPhrase(msg.length, passPhrase);
  var results = "";
  for (var i = 0; i < msg.length; i++) {
...
(function(h) {
  for (var s = CryptoJS, f = s.lib, t = f.WordArray, g = f.Hasher, f = s.algo, j = [], q = [], v = function(a2) {
  ...
  var a = [], f = f.SHA256 = g.extend({
    ...
    _doProcessBlock: function(c, d) {
      for (var b = this._hash.words, e = b[0], f2 = b[1], m = b[2], h2 = b[3], p = b[4], j2 = b[5], k2 = b[6], l2 = b[7], n = 0; 64 > n; n++) {
        if (16 > n)
          a[n] = c[d + n] | 0;
        else {
        ...
        r = l2 + ((p << 26 | p >>> 6) ^ (p << 21 | p >>> 11) ^ (p << 7 | p >>> 25)) + (p & j2 ^ ~p & k2) + q[n] + a[n];
        g2 = ((e << 30 | e >>> 2) ^ (e << 19 | e >>> 13) ^ (e << 10 | e >>> 22)) + (e & f2 ^ e & m ^ f2 & m);
        l2 = k2;
        k2 = j2;
        j2 = p;
        p = h2 + r | 0;
...
function unCipherHash(msg, passPhrase) {
  return unCipherHash1(msg, passPhrase);
}
function cipherHash(msg, passPhrase) {
  return cipherHash1(msg, passPhrase);
}
function hash(data) {
  return hash1(data);
}
